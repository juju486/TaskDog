...
const addOneGroup = async () => {
  ...
  try {
    await configApi.addGroup({ type: 'task', name })
...
const renameOne = async (oldName) => {
  try {
    ...
    await configApi.renameGroup({ type: 'task', oldName, newName })
...
const deleteOne = async (name) => {
  try {
    await ElMessageBox.confirm(`删除分组 "${name}"？该分组下的任务将不再归属任何分组。`, '删除分组', { type: 'warning', confirmButtonText: '删除', cancelButtonText: '取消' })
    await configApi.deleteGroup({ type: 'task', name })
